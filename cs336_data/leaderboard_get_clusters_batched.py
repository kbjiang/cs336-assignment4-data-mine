...
if __name__ == "__main__":
    ...
    # Use as_completed (best for progress tracking)
    print("\n# Step 2: generating duplicate candidates by band")
    band_size = 16
    num_bands = sigs.shape[1] // band_size

    # Parallel processing across bands - submit in batches to reduce startup overhead
    n_workers = len(os.sched_getaffinity(0))
    candidates = []
    batch_size = n_workers * 4  # Process 4x workers at a time

    with ThreadPoolExecutor(max_workers=n_workers) as executor:
        for batch_start in range(0, num_bands, batch_size):
            batch_end = min(batch_start + batch_size, num_bands)
            futures = [executor.submit(get_candidates_single_band, sigs, i) 
                      for i in range(batch_start, batch_end)]
            
            for future in tqdm(as_completed(futures), 
                             desc=f"Bands {batch_start}-{batch_end}", 
                             total=len(futures)):
